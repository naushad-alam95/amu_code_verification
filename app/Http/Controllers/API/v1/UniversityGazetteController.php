...
use App\UniversityGazette;
use App\Widget;
use Validator;
use Auth;
use DB;

class UniversityGazetteController extends BaseController
{
    ...
    public function index(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            try {
                $gazette = UniversityGazette::where('status','1')->orderBy('order_on','asc')->get();
                /*$datawidget = array();  
                ...
                switch ($locale) {
                    case "ur": //GET Annual Report IN Urdu
                        $dataUR = array();
                        if (empty($gazette)){                        
                            return $this->sendResponse($gazette, trans($locale.'_lang.DATA_NOT_FOUND'),404);
                        }                        
                        /*if (!empty($widget)) {
